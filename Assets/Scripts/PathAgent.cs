...
public class PathAgent : MonoBehaviour
{
    ...
    public float speed;
    protected static float NEARBY = 0.05f;
    protected static System.Random rnd = new System.Random();

    protected int counter = 0;

    protected Animator aniCont;
    ...
    void Start()
    {
        ...
        aniCont = GetComponentInChildren<Animator>();
    }
    ...
    void Update()
    {
        ...
        if (path.Count == 0)
        {
            ...
            Vector3 dir = new Vector3(here.x - there.x, 0f, here.z - there.z);

            if (!walking)
            {
                walking = true;
                aniCont.SetBool("walking", true);
            }
            // Are we there yet?
            ...
            if (distance < NEARBY)
            {
				...
				//transform.LookAt (new Vector3 (getX (path [0], 40), 0f, getY (path [0], 40)));
